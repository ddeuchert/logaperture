...
**`--include-sticky`** (kebab-case, matching the CLI's existing flag convention — `--include-children`, `--json` — renamed from the `--ignore-sticky` originally proposed here, to read as "include the sticky ones" rather than "ignore stickiness"). Every form above takes it. **New default: reset skips `STICKY`-tier overrides** — a sticky override is a considered, "known-noisy, I decided about it" decision (§6.1) that a routine or broad reset shouldn't silently undo — and `--include-sticky` is the explicit escape hatch. This is a genuine behavior change from what's currently shipped and documented; `persistence.md`'s exit-criterion sentence about sticky removal describes the pre-this-item behavior and needs a forward-pointing note once this lands, not a rewrite of what that milestone actually verified at the time.

**Resolved:**

- `logctl reset --all` is **dropped**, not kept — `reset loggers` + `reset handlers` are the only broad forms; nothing reverts both namespaces in one command after this lands.
- Naming one specific target (`reset logger <exact-name>`, `reset handler <name>`) that turns out to be sticky, without `--include-sticky`, is a **refusal** (non-zero exit, nothing mutated) — not a silent no-op. Naming a *set* (`reset logger <pattern>`, `reset loggers`, `reset handlers`) leaves sticky members alone and reports them, since "the pattern/bulk matched nothing to touch" was never an error either. Full write-up: [`reset-command-surface.md`](specs/reset-command-surface.md).
