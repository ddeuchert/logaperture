...
- **Glob on apply** (`logctl error *.infinispan`). A fan-out layer that resolves the pattern against known loggers and applies the level to each match.
  ...
  **Open questions, deferred until this is specced:** precedence when a logger matches more than one standing pattern; interaction with an explicit exact-name override on a logger that also matches a standing pattern (the exact-name override should win, but the rule needs stating); the confirmation-flag's exact name and how apply-side prompting composes with `logctl reset --all`'s existing no-prompt convention ([`cli-transport.md`](specs/cli-transport.md) — that escape hatch deliberately never prompts, but an *amplifying, indefinite* standing rule is the opposite kind of risk); one audit record per matched logger and a capability check per match (mechanical, but needs pinning down); the persistence/state-file shape for a standing rule, which is a new kind of persisted entity, not just another override row; whether `logctl` should accept multiple pattern/name targets in one call (`logctl error org.apache org.apache.*`) as a convenience for the now-two-call "self and descendants" case. Layer 2.
...
### 18.9 Reset command surface: logger/handler split + `--ignore-sticky`
...
`logctl reset handler <name>` replaces `logctl handler <name> reset` — a deliberate breaking change, acceptable pre-1.0, even though the replaced form already shipped across PR #12/#19/#30. `logctl handler <name> <level>` (the level-*setting* form) is untouched; only the reset-shaped commands move. `reset handler`/`reset handlers` stay exact-match only — handler names are short and not deeply-nested the way logger categories are, so the lookup-then-copy friction that motivates logger globbing (§18.7) doesn't apply here.

**Ownership split with §18.7 / #41**, so neither item duplicates the other: #41 owns the pattern-matching mechanics (the glob matcher, and the standing-rule semantics for the *apply* side); `logctl reset logger <pattern>` is this item's command spelling of #41's already-planned "`reset <pattern>` symmetry," reusing #41's matcher rather than building a second one. This item owns the command-surface split and `--ignore-sticky`, which apply to every reset form, not just pattern-based ones.

**`--ignore-sticky`** (kebab-case, matching the CLI's existing flag convention — `--include-children`, `--json` — not the camelCase originally proposed). Every form above takes it. **New default: reset skips `--sticky`-tier overrides** — a sticky override is a considered, "known-noisy, I decided about it" decision (§6.1) that a routine or broad reset shouldn't silently undo — and `--ignore-sticky` is the explicit escape hatch meaning "include the sticky ones in this reset too." This is a genuine behavior change from what's currently shipped and documented; `persistence.md`'s exit-criterion sentence about sticky removal describes the pre-this-item behavior and needs a forward-pointing note once this lands, not a rewrite of what that milestone actually verified at the time.
