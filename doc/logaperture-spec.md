...
**Pulled forward: an environment report for bug reports (`logctl env`).** A read-only command that prints the facts a good issue report needs, so a user filing one can paste a single block instead of being asked follow-up questions: Java version and vendor, OS and architecture, the active logging backend and its version, the detected application framework or container and its version, the LogAperture agent and `logctl` build versions themselves, and the fully-qualified path of the file this JVM persists overrides to. Sibling to `top` and `doctor` in Layer 1 — read-only, no rule engine, only the `VIEW` capability (§9.3) — but aimed at *"paste this into the issue"* rather than at diagnosing configuration, so it stays a separate command rather than a `doctor` finding. Alpha-2; tracked as [#32](https://github.com/ddeuchert/logaperture/issues/32). Spec: [`doc/specs/environment-report.md`](specs/environment-report.md) — real backend/container version detection ships for JBoss LogManager and WildFly this slice; Logback and every other container are a fast-follow (Decision #6).
...
**Pulled forward: reset command surface split + `--ignore-sticky`.** `logctl reset` and `logctl handler <name> reset` grew independently and now can't scope a reset to just loggers or just handlers, and can't protect a deliberately-set `--sticky` override from a broad reset. Restructures reset into `reset logger <pattern>` / `reset loggers` / `reset handler <name>` / `reset handlers`, all taking `--ignore-sticky` (new default: sticky is skipped unless asked for) — a breaking rename of the already-shipped `handler <name> reset`, accepted pre-1.0. Reuses #41's glob matcher for the logger form rather than duplicating it. Full write-up and open questions: §18.9; tracked as [#42](https://github.com/ddeuchert/logaperture/issues/42).
...
The rendered log line often shows only an abbreviated category — WildFly's default console pattern renders `%c{1}`, so a noisy line reads `infinispan`, not `org.jboss.as.clustering.infinispan`. Quieting that noise today means manually resolving the short name to its fully-qualified logger (cross-referencing message-id tables, or turning on a fuller pattern) before `setLevel` will accept it. Two complementary reductions, both pulled forward to alpha-2 and tracked as [#41](https://github.com/ddeuchert/logaperture/issues/41):

- **Glob on lookup** (`logctl levels *.infinispan`). **Shipped; grammar tightened in slice 1 of #41 below.** The `listLoggers(filter)` glob is specified in [`doc/specs/level-control.md`](specs/level-control.md) and [`doc/specs/cli-transport.md`](specs/cli-transport.md), documented in `logctl --help`, and covered end-to-end by tests.
- **Glob on apply** (`logctl error *.infinispan`). A fan-out layer that resolves the pattern against known loggers and applies the level to each match.

  **Scope decision: a standing rule, not one-shot.** The pattern isn't just resolved once against loggers live at call time — it persists, and is applied automatically to loggers discovered afterward too (e.g. a scanner class not yet loaded when the command runs). This does *not* mean pulling the full squelch engine (§7 / Feature 3) forward: no gate/render actions, no per-event counters, no burst detection. It's narrowly *declarative desired level per pattern, persisted, (re-)applied whenever a not-yet-seen logger is discovered that matches it, until reset* — reusing machinery that already exists rather than building new: the glob matcher above, the override/expiry/persistence tiers (Feature 2), and the periodic sweep thread already used to expire overrides (`sweepExpiredOverrides` in `LevelControlService` / `AggregateLevelControl` / `HandlerLevelControlService`, driven by `SweepPolicy`) — extending that same sweep pass to also check newly-discovered loggers against standing patterns is architecturally cheap and needs none of Feature 3's event pipeline.

  **`logctl error <pattern>` previews before applying** — it lists the resolved matches, states that it will also apply to future matches (since it's a standing rule), and asks for confirmation; a flag (name deferred to the spec, e.g. `--yes`) skips the prompt for scripts. **`logctl reset <pattern>` gets the same glob support**, and, since a standing pattern is a persisted rule rather than a one-time application, reset on a pattern retires the rule as well as reverting its current matches — leaving a "zombie" rule that silently reapplies itself later would be a surprising trap. (§18.9 / [#42](https://github.com/ddeuchert/logaperture/issues/42) works out the reset-side command spelling this reaches through.)

  **Grammar — decided, not deferred, and shipping as slice 1.** A pattern is at most one leading `*.` and/or one trailing `.*`, every segment between them plain literal text, nothing else: `org.apache.*`, `*.apache.writer`, `*.apache.writer.*` are valid; `org.*apache` (wildcard mixed into a literal segment), a wildcard segment in the middle (`org.*.writer`), bare `*`, and `*.*` are all rejected as usage errors. `?` is dropped too — it's the same partial-segment mechanism as mid-segment `*`, banned for the same reason. This one grammar replaces the shipped lookup grammar as well as governing the later apply/reset commands (#41, #42) — no second, more permissive grammar kept around for lookup. The restriction isn't just a safety margin: every real abbreviation pattern (Logback's `%logger{length}`, JBoss's `%c{1}`) elides whole *leading* segments and keeps the trailing ones intact, never truncates mid-segment — so segment-anchored matching is the shape the actual problem already has, not an arbitrary limitation on top of a more general feature. Self-contained (`listLoggers`/`NameFilter` only, no API or MXBean change), so it ships alone first: see [`level-control.md`](specs/level-control.md)'s `listLoggers(filter)`.

  A leading/trailing wildcard matches the named logger itself and **zero or more** further segments — `org.apache.*` matches `org.apache` and every descendant of it. This makes it a true superset of today's `includeChildren` (which also covers the named logger and its descendants in one call), not merely an approximation of it: a caller wanting "self and descendants" needs only the one pattern target, not two. (An earlier draft of this section specified one-or-more, never zero, matching a strict-descendants-only reading; slice 1 shipped that way and slices 2–3's design work surfaced that it left no single-target way to express "self and descendants" — corrected here, and in the already-merged `NameFilter`, before slices 2–3 build on top of it.) **`includeChildren` is retired from the operations API entirely** (not just the CLI flag) — but only once the standing-rule apply mechanism below actually ships (a later slice), not alongside the grammar. Retiring it earlier would leave a gap where neither mechanism covers "this logger's descendants" — the grammar and the `includeChildren` replacement are deliberately sequenced apart for exactly this reason. When it does land: one mechanism, glob patterns, for every caller, JMX included; a breaking change to `setLevel`'s wire signature (removes a parameter, not additive) — see the "Superseded (planned, not this slice)" note in [`level-control.md`](specs/level-control.md) and top-level §11.1's component-versioning policy for the MXBean-signature consequence (moot pre-1.0 per §11.1's "no skew guarantee before 1.0," but still worth calling out explicitly rather than incidentally).
...
### 18.9 Reset command surface: logger/handler split + `--include-sticky`

Today's reset surface is split in an accidental way rather than a useful one. `logctl reset <logger>` reaches one logger by exact name; `logctl reset --all` reverts *every* override, loggers and handlers together, with no way to scope to one namespace; `logctl handler <name> reset` reaches one handler, deliberately kept in a separate namespace ([`handler-floor-control.md`](specs/handler-floor-control.md): "handler resets are always spelled `handler <name> reset` to keep the two namespaces distinct"). There is no way to say "reset every logger override but leave my handler overrides alone" (or the reverse), and no way to protect a deliberately-set `--sticky` override from a broad reset — today `resetLevel`/`resetAll` remove a sticky override exactly like any other tier ([`persistence.md`](specs/persistence.md)'s M1 exit criterion: "removed only by an explicit `resetLevel`/`resetAll`," no special-casing). Pulled forward to alpha-2; tracked as [#42](https://github.com/ddeuchert/logaperture/issues/42).

**Specced and signed off:** [`doc/specs/reset-command-surface.md`](specs/reset-command-surface.md), all six design decisions resolved. Summary below; that document is the source of truth until implementation lands, at which point this section gets the same "shipped" treatment §18.7 above already has.

**Command shape — space-separated subcommands under `reset`, not new top-level verbs**, for the same phone-test reason today's commands are already spelled as words rather than run-together identifiers (cli-transport.md "The phone test, enforced"):

```
logctl reset logger <pattern>     # one logger, or every logger matching a glob (§18.7 / #41)
logctl reset loggers              # every logger override — loggers only
logctl reset handler <name>       # one handler, exact name only — no glob
logctl reset handlers             # every handler override — handlers only
logctl reset --all                # both namespaces at once — kept, not replaced by the two above
```

`logctl reset handler <name>` replaces `logctl handler <name> reset` — a deliberate breaking change, acceptable pre-1.0, even though the replaced form already shipped across PR #12/#19/#30. `logctl handler <name> <level>` (the level-*setting* form) is untouched; only the reset-shaped commands move. `reset handler`/`reset handlers` stay exact-match only — handler names are short and not deeply-nested the way logger categories are, so the lookup-then-copy friction that motivates logger globbing (§18.7) doesn't apply here. The bare, keyword-less `logctl reset <name>` form is retired too — every reset now names its namespace explicitly.

**Ownership split with §18.7 / #41**, so neither item duplicates the other: #41 owns the pattern-matching mechanics (the glob matcher, and the standing-rule semantics for the *apply* side); `logctl reset logger <pattern>` is this item's command spelling of #41's already-planned "`reset <pattern>` symmetry," reusing #41's matcher rather than building a second one. This item owns the command-surface split and `--include-sticky`, which apply to every reset form, not just pattern-based ones.

**`--include-sticky`** (kebab-case, matching the CLI's existing flag convention — `--include-children`, `--json`; renamed from the originally-proposed `--ignore-sticky` during spec review — since the *default* is already to skip sticky, a flag called "ignore sticky" read as reinforcing that default rather than overriding it). Every form above takes it, including a single, fully-named sticky target — naming a sticky override's exact name is not itself consent to touch it. **New default: reset skips `--sticky`-tier overrides** — a sticky override is a considered, "known-noisy, I decided about it" decision (§6.1) that a routine or broad reset shouldn't silently undo — and `--include-sticky` is the explicit escape hatch. This is a genuine behavior change from what's currently shipped and documented; `persistence.md`'s exit-criterion sentence about sticky removal describes the pre-this-item behavior and needs a forward-pointing note once this lands, not a rewrite of what that milestone actually verified at the time.

**Partial reset of a standing rule (new capability, surfaced during spec review, beyond the issue's original scope).** #41's shipped `resetLevel(target)` only knows two states for a standing rule — fully active, or fully retired by typing back its exact original pattern string — with no way to revert just *part* of what it covers. `reset-command-surface.md` closes that gap: a `PatternRule` gains a persisted exclusion set (same grammar as its own pattern), and `resetLevel(target)` becomes one algorithm for both full and partial reset — full-string retirement (today's behavior) is just the case where nothing is left over after carving `target` out. An excluded exact name's own descendants keep inheriting the rule; an excluded sub-pattern takes its whole subtree with it, present and future. The design goal throughout: `reset logger <target>` always does the right thing for whatever currently governs `target`, without the caller needing to know whether that's a rule, a plain override, or nothing — matching `reset`'s existing job of getting back to baseline without reconstructing history (§14.5's deferred `logctl undo` is the one that would need that).

No confirmation/preview on any reset form, pattern included — reverting a bounded, current state is the opposite risk shape from applying an unbounded, future-reaching one (the same asymmetry #41 already established for `reset <pattern>`).
