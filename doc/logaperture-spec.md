...
- **Glob on lookup** (`logctl levels *infinispan*`). **Shipped, grammar changing.** The `listLoggers(filter)` glob accepts `*` anywhere including leading, plus `?` for a single character; specified in [`doc/specs/level-control.md`](specs/level-control.md) and [`doc/specs/cli-transport.md`](specs/cli-transport.md), documented in `logctl --help`, and covered end-to-end by tests. #41 replaces this grammar rather than leaving it as-is — see "Grammar" below.
- **Glob on apply** (`logctl error *.infinispan`). A fan-out layer that resolves the pattern against known loggers and applies the level to each match.
  ...
  **Grammar — decided, not deferred.** A pattern is at most one leading `*.` and/or one trailing `.*`, every segment between them plain literal text, nothing else: `org.apache.*`, `*.apache.writer`, `*.apache.writer.*` are valid; `org.*apache` (wildcard mixed into a literal segment), a wildcard segment in the middle (`org.*.writer`), bare `*`, and `*.*` are all rejected as usage errors. `?` is dropped too — it's the same partial-segment mechanism as mid-segment `*`, banned for the same reason. This one grammar replaces the shipped lookup grammar as well as governing the new apply/reset commands (#41, #42) — no second, more permissive grammar kept around for lookup. The restriction isn't just a safety margin: every real abbreviation pattern (Logback's `%logger{length}`, JBoss's `%c{1}`) elides whole *leading* segments and keeps the trailing ones intact, never truncates mid-segment — so segment-anchored matching is the shape the actual problem already has, not an arbitrary limitation on top of a more general feature.

  A leading/trailing wildcard matches **one or more** segments, never zero — `org.apache.*` matches descendants of `org.apache`, never `org.apache` itself. This is *not* a strict superset of today's `includeChildren` (which covers the named logger and its descendants in one call); a caller wanting both targets `org.apache` and `org.apache.*`. **`includeChildren` is retired from the operations API entirely** (not just the CLI flag) once this ships — one mechanism, glob patterns, for every caller, JMX included. That's a breaking change to `setLevel`'s wire signature (removes a parameter, not additive) and to the shipped `listLoggers` grammar; see the "Superseded (planned)" notes in [`level-control.md`](specs/level-control.md) and [`cli-transport.md`](specs/cli-transport.md), and top-level §11.1's component-versioning policy for the MXBean-signature consequence.
