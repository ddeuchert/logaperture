...
> **Superseded (shipped).** The paragraph above describes the surface as originally
> shipped. [`reset-command-surface.md`](reset-command-surface.md) (issue #42) replaced
> `logctl handler <name> reset` with `logctl reset handler <name>` — a deliberate
> breaking rename, not a kept synonym — as part of splitting `reset` into
> `logger`/`loggers`/`handler`/`handlers` forms, each taking `--include-sticky` (every
> reset now leaves a `STICKY`-tier override alone by default). `logctl reset --all` still
> reverts both namespaces in one call. `logctl handler <name> <level>` (setting, as
> opposed to resetting) is unaffected.
