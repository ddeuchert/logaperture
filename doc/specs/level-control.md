...
|---|---|---|---|
| `includeChildren` | boolean | `false` | Mirrors Logback's own hierarchy semantics (§5) — does not change how the level applies, only whether it is also applied to loggers already known to be descendants at call time. **Superseded — removed; see [`pattern-level-targeting.md`](pattern-level-targeting.md):** a trailing-wildcard pattern (`org.apache.*`, matching `org.apache` itself and zero-or-more descendant segments — a true superset of this option's own semantics) becomes the one mechanism for "this logger and its descendants," for every caller, CLI or JMX. |
