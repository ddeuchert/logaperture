 ...
 * action type ({@link Drop}, #34's {@code Trim}) supplies its own
 ...
 *
 * @implNote {@code payload} is {@code Map.of()} on a fresh attach (the
 * caller's own factory closes over whatever action-specific state it needs
 * instead — see {@link Drop#attachFactory}) and the persisted row's {@link
 * LogRule#persistedPayload()} on resume — doc/specs/drop-rule.md
 * Decision #4.
 */
@FunctionalInterface
public interface RuleFactory {
    ...
    LogRule create(String id, String loggerName, CompiledMatchers matchers, String reason, PersistenceTier tier,
            Instant expiresAt, Instant createdAt, Map<String, String> payload);
}
